...
export type GetAuditServiceContractParams = z.infer<
  typeof getAuditServiceContractSchema
>;
...
export async function getAuditServiceContract(
  params: GetAuditServiceContractParams
) {
  const response = await dartRequest(
...
export type GetNonAuditServiceContractParams = z.infer<
  typeof getNonAuditServiceContractSchema
>;
...
export async function getNonAuditServiceContract(
  params: GetNonAuditServiceContractParams
) {
  const response = await dartRequest(
    buildUrl(
      "https://opendart.fss.or.kr/api/accnutAdtorNonAdtServcCnclsSttus.json",
      params
    )
  );
  const data = await response.json();
